...
\begin{proof}
 ...
 \parpic[r]{\begin{tikzpicture}
   \node (p_0) {$p_0$}
    ...
    child {node (q_1) {$q_1$}};
   \node[right=1cm of p_0] (q_0) {$q_0$};
   \tikzmath{
     coordinate \begn, \dest;
     \begn = (q_0);
     \dest = (q_3);
     real \longi, \dx, \dy, \angl;
     \dx = \begnx - \destx;
     \dy = \begny - \desty;
     \longi = veclen(\dx, \dy) / 2 + 5mm;
     \angl = atan(\dy / \dx);
   }
   \begin{pgfonlayer}{background}
    \path[fill=gray, opacity=.5,rotate=\angl]
     ($(q_0)!.5!(q_3)$) ellipse (\longi pt and 5mm);
   \end{pgfonlayer}
   \node at ($0.5*(p_0)+0.5*(q_0)$) {$\perp$};
   \node at ($0.5*(p_1)+0.5*(q_1)$) {$\perp$};
   \node at ($0.5*(p_2)+0.5*(q_2)$) {$\perp$};
   \node[below right of=q_2] {$\mathcal{A}$};
 \end{tikzpicture}}
