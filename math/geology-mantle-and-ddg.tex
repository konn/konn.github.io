...
\begin{abstract}
 ...
 \end{enumerate} 
 \emph{集合論の地質学}は，与えられた集合論の宇宙$V$の内部モデルがいかなる生成拡大になっているかを考える集合論の分野ですが，今回は薄葉~\cite{Usuba:2017fp}を種本にその基本定理である\emph{下方有向性原理}を示し，その系として全ての基礎モデルの共通部分である\emph{マントル}$\mantle$が強制法で不変な内部モデルであり，更に$\ZFC$を満たすことを見ます．
\end{abstract}
...
\end{theorem}
\begin{proof}
 原論文~\cite{Usuba:2017fp}か\href{http://konn-san.com/math/geology-ground-definability.html}{前回の記事}~\cite{ISHII:2017jx}の定理2を参照． \qed
\end{proof}
