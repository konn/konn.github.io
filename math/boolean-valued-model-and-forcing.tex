...
\setmathfont[range={\setminus,\models}]{Asana Math}
\setmathfont[range={bb}]{TeX Gyre Pagella Math}
\setmathfont[range={up,sfup,cal}]{latinmodern-math.otf}
...
$V^{\mathbb{B}}$という物を考えたのは，$V$にはない元を付加するためでした．
...
$D$を稠密集合とし，$d \defeq \sum D$と置きます．
この時，$d < 1$ならば$0 < -d$となりますが，$D$の稠密性から$e \in D$で$0 < e \leq -d$となる物が必要です．
しかし，$d$は$D$の上限なので$e \leq \sum D = d$となり，$e \cdot d \leq -d \cdot d = 0$となり矛盾します．
よって$D$が稠密なら$\sum D = 1$となることがわかります．
...
\parpic[r]{\begin{tikzpicture}[scale=.8]%
   ...
   \node (om) [font=\scriptsize,fill=white] at (0.4, 0.75) {$\omega$};
   % delta, omega_1
    \path[draw=black,fill=transparent] (-0.1, 2) edge (0.1, 2);
    \node (omega1) [font=\scriptsize,fill=white] at ($(0.4,2)+(.2,0)$) {$\omega_1$};
   % V[G]
