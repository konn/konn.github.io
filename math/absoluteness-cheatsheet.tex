...
\begin{definition}
 ...
 \begin{enumerate}
  ...
  \item $\Sigma^1_n \defeq \Sigma^1_n(\emptyset), \Pi^1_n \defeq \Pi^1_n(\emptyset), \Delta^1_n \defeq \Delta^1_n(\emptyset), $
  \item $\boldface{\Sigma}^1_n \defeq \bigcup_{a \in \Baire} \Sigma^1_n(a), \qquad
        \boldface{\Pi}^1_n \defeq \bigcup_{a \in \Baire} \Pi^1_n(a), \qquad
        \boldface{\Delta}^1_n \defeq \boldface{\Pi}^1_n \cap \boldface{\Sigma}^1_n.$
 \end{enumerate}
\end{definition}
\begin{theorem}[Mostwski Absoluteness]
 $\boldface{\Pi}^1_1$-概念は任意の推移的モデルについて絶対．
 従って$\boldface{\Delta}^1_1$と$\boldface{\Sigma}^1_1$もそう．
\end{theorem}

\begin{proof}
 $M$をc.t.m.とし，$A \in M$を$\Pi^1_1$-概念とする．
 特に，算術的論理式$\varphi(x)$について，$x \in A \iff \forall z \in \Baire \: \varphi(z, x)$と書けているとしよう．\qed
\end{proof}

\begin{theorem}[Shoenfield Absoluteness]
 $\omega_1 \in M$を満たす$\ZF$の十分大きな有限断片を満たす部分推移的モデルについて，$\boldface{\Pi}^1_2$-概念（よって$\boldface{\Delta}^1_2$および$\boldface{\Sigma}^1_2$概念も）は絶対．
