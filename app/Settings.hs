...
instance FromJSON NavBar where
  parseJSON = withArray "nav array" $ \vs -> NavBar <$> mapM p (toList vs)
    where
      p (Object d) | [(k, v)] <- HM.toList $ KM.toHashMapText d = (,) k <$> parseJSON v
      p _ = empty
...
instance FromJSON Card where
  parseJSON val =
    withText "Template String" (fmap (flip Card False) . fromMus) val
      <|> withObject
        "Template Dict"
        ( \obj ->
            Card <$> (fromMus =<< obj .: "template")
              <*> obj .: "gather"
        )
        val
    where
      fromMus txt =
        either
          (fail . MP.errorBundlePretty)
          return
          $ Mus.compileMustacheText "url" txt

data Cards = Cards
  { cardDic :: HashMap T.Text Card
  , defaultCard :: Card
  }
...
instance FromJSON Cards where
  parseJSON = withObject "Card dictionary" $ \dic0 -> do
    ...
    d <- asum $ return <$> KM.lookup "*" dic
    return $ Cards (HM.delete "*" $ KM.toHashMapText dic) d
