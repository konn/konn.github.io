  ...
  parseJSON obj =
    ...
    where
      parsePairs hm = do
        ...
        return $ map (uncurry LinkItem) $ HM.toList dic
      parsePair hm = do
        [(l, t)] <- HM.toList <$> parseJSON hm
        return $ LinkItem l t
data Conference =
  Conference
    { cnfName  :: Text
...
instance FromJSON Conference where
  parseJSON j = withText "Conference name expected"
               (\t -> pure $ Conference t Nothing Nothing) j
            <|> genericParseJSON cnfOption j

addItemInfo :: Talk -> Value
addItemInfo talk@Talk{..} =
  let Object dic = toJSON talk
  in Object $ HM.insert "hasLinks" (Bool $ not $ null links)
      $ HM.insert "links" (toJSON $ links <&> \ ls ->
...
addLinkInfo maxLen pos li =
  ...
  in toJSON $ HM.insert "notLast" (Bool $ pos < maxLen - 1) dic
