...
# 更新履歴
* 2017/09/06 13:40:00 JST [同期の不具合](#sync)について訂正・追記。
...
### <ins>同期不具合の修正方法</ins> ~~同期はUSBの信頼性に若干疑問~~（**2017/09/06追記**） {#sync}
...
<ins>\[20170906追記]</ins>
サポートに連絡してみたところ、確認方法の手引きの書いたPDFを頂いた。
どうも、DPT-RP1がUSB経由で同期出来るためには、**ネットワークサービスとしてシステムに登録されている必要がある**ようだ。
プロクシの設定とかを環境に応じて変更する必要があるので、私は [Control Plane](https://www.controlplaneapp.com) を使って、最近傍のWiFiネットワークによってネットワーク環境を自動で切り替えるようにしていた。

私の場合、DPT-RP1をセットアップしたのは院生室だったので、**初回同期時に院生室用のネットワーク環境にDPT-RP1の設定がインストール**されたようだ。
しかし、**院生室以外では「自動」プロファイルに自動変更**するようにしてあるので、**そちらではDPT-RP1を認識出来ず**、USB経由での同期が出来なくなっていたらしい。
アプリを再インストールした後だったが、再度ネットワークサービスにDPT-RP1が追加されたことで、「自動」環境でもひとまず動くようになった。

こうした事は問い合わせて貰えるPDFを読んで始めてわかる。
どうせ資料として用意されているのだから、**最初から接続ガイドに記載しておいてほしい**。
「ネットワーク環境を弄ってたら壊れた！」って言われないためだろうか？まあ、よくわからない。  
<ins>\[/20170906追記終わり]</ins>
