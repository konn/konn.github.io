---
title: Boole値モデルと強制法
date: 2022/06/11 18:00:00 JST
author: 石井大海
description: 集合論における無矛盾性証明で用いられる主要な手法である**強制法**と，密接に関連する**Boole値モデル**の手法について，本稿では幾らか証明を省略しつつ概略を採り上げます．また，Hamkinsら [@Hamkins:2012qv] の説明に基づいて，超冪とBoole値モデルの関係についても簡単に解説します．
latexmk: -pdflua
...
どういう事でしょうか？
まず，一般の集合$x \in V$は，特性関数と同一視することで，部分関数$x: V \dashrightarrow 2$と見做すことが出来ます．
...
\begin{definition}
 ...
 \end{itemize}
\end{definition}
\begin{remark}[パラメータが真のクラスを渡る上限・下限について]
  鋭い人は，上の真偽値の定義の$\forall$の部分に違和感を覚えるかもしれません：
  \begin{align*}
    \truth{\forall x \: \varphi[x, \vec{\sigma}]}
    \defeq \prod_{\dot{x} \in V^{\mathbb{B}}} \truth{\varphi[\dot{x}, \vec{\sigma}]}.
  \end{align*}
  というのもここで，$\prod$が$V^{\mathbb{B}}$という\emph{真のクラス}を亘って取られているように見えるからです．
  完備Boole代数では，$\prod$や$\sum$の範囲は\emph{集合}に限定されていた筈だが，これは大丈夫なのか？と心配になるわけです．
  しかし，ここで$\prod$はあくまで$\mathbb{B}$の\emph{特定の条件を満たす要素たち}に対してとっており，$\mathbb{B}$自体は\emph{集合}ですので，最終的に下限が取られる対象も集合に収まるので問題はありません．

  もう少し詳しく説明すると，以下のようになります．
  まず，上記の$\prod_{\dot{x} \in V^{\mathbb{B}}} \truth{\varphi[\dot{x}, \vec{\sigma}]}$というのは以下の略記であったことを思い出しましょう：
  \begin{align*}
    &\prod_{\dot{x} \in V^{\mathbb{B}}} \truth{\varphi[\dot{x}, \vec{\sigma}]}\\
    &= \prod \Set{ \truth{\varphi[\dot{x}, \vec{\sigma}]} | \dot{x} \in V^{\mathbb{B}}}\\
    &= \prod \Set{ b \in \mathbb{B} | \exists \dot{x} \in V^\mathbb{B} \: b = \truth{\varphi[\dot{x}, \sigma]} }
  \end{align*}
  最後の式で$\prod$が取られているクラスは，ある論理式を満たす$\mathbb{B}$の元の全体，という形で定義されています．
  いま，$\mathbb{B}$そのものは集合でしたから，\emph{分出公理}よりこの形のクラスも集合であることが保証されます．
  よって，$\mathbb{B}$の完備性からその下限を取ることができ，無事定義がwell-definedになる訳です．

  このように，$\prod, \sum$をとる際に使われるパラメータが真のクラスであっても，$\mathbb{B}$が集合を成すことに気を付ければ自由に上限・下限を取ることができるので，以下気にせず取ってしまうことにします．
\end{remark}
...
\begin{theorem}
 ...
 特に，有限集合，$\omega$である，関数である，順序数である，といった性質は動かない．
\end{theorem}
...
\begin{itemize}
  ...
  \item 2022/06/11 18:00:00 JST 推移的モデルについて絶対的な概念として誤って「可算である」を挙げていたので削除。
\end{itemize}
